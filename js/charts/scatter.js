...
export function build(sel, props){
  const W=1200,H=700,M={t:30,r:30,b:74,l:84},svg=createSVG(sel,W,H),g=svg.append('g').attr('transform',`translate(${M.l},${M.t})`);
  ...
  const dots=g.append('g').selectAll('circle').data(pts).join('circle').attr('cx',d=>x(d.dist)).attr('cy',innerH).attr('r',3.2).attr('fill','rgba(123,223,242,.9)');
  ...
  const reg=d3.line().x(d=>x(d)).y(d=>y(intercept+slope*d)), domain=[x.domain()[0], x.domain()[1]];
