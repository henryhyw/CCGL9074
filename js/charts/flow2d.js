...
export async function build(sel, props){
  ...
  nodeG.selectAll('circle.gen').data(props.gens).join('circle').attr('class','gen')
    .attr('cx',d=>projection([d.lon,d.lat])?.[0]).attr('cy',d=>projection([d.lon,d.lat])?.[1]).attr('r',3.5).attr('fill','#fff');
  nodeG.selectAll('circle.hub').data(props.hubs).join('circle').attr('class','hub')
    .attr('cx',d=>projection([d.lon,d.lat])?.[0]).attr('cy',d=>projection([d.lon,d.lat])?.[1]).attr('r',4.5).attr('fill','var(--brand)');
  nodeG.selectAll('text.lbl').data([...props.hubs,...props.gens]).join('text').attr('class','lbl').attr('x',d=>projection([d.lon,d.lat])?.[0]+6).attr('y',d=>projection([d.lon,d.lat])?.[1]-6)
