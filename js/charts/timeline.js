...
export function build(sel, props){
  const W=1200,H=520,M={t:30,r:40,b:70,l:90},svg=createSVG(sel,W,H),g=svg.append('g').attr('transform',`translate(${M.l},${M.t})`);
  ...
  const bars=g.append('g').selectAll('rect').data(items).join('rect').attr('x',x(0)).attr('y',d=>y(d.name))
    .attr('width',0).attr('height',y.bandwidth()).attr('rx',8).attr('fill',d=>d.color || 'var(--ok)');
