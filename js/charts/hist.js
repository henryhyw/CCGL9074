...
export function build(sel, props){
  const W=1200,H=600,M={t:30,r:24,b:88,l:70},svg=createSVG(sel,W,H),g=svg.append('g').attr('transform',`translate(${M.l},${M.t})`);
  ...
  g.append('g').call(d3.axisLeft(y).ticks(6).tickSize(-innerW))
    .call(g=>g.selectAll('text').attr('fill','var(--muted)')).call(g=>g.selectAll('line,path').attr('stroke','var(--gridline)'));
  g.append('text').attr('class','axis-title').attr('x',innerW/2).attr('y',innerH+60).attr('text-anchor','middle').text(props.xLabel||'Value');
  g.append('text').attr('class','axis-title').attr('transform',`translate(${-50},${innerH/2}) rotate(-90)`).attr('text-anchor','middle').text(props.yLabel||'Count');
