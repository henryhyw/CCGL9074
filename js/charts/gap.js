...
export function build(sel, props){
  const W=1200,H=700,M={t:40,r:40,b:72,l:82},svg=createSVG(sel,W,H),g=svg.append('g').attr('transform',`translate(${M.l},${M.t})`);
  ...
  g.append('text').attr('class','axis-title').attr('transform',`translate(${-54},${innerH/2}) rotate(-90)`).attr('text-anchor','middle').text(props.yLabel||'MW');
  const line=d3.line().x(d=>x(d.x)).y(d=>y(d.y)).curve(d3.curveMonotoneX);
  const area=d3.area().x(d=>x(d.x)).y0(d=>y(props.seriesB.find(c=>c.x===d.x).y)).y1(d=>y(d.y)).curve(d3.curveMonotoneX);
  if(props.fillGap!==false){
    g.append('path').datum(props.seriesA).attr('d',area).attr('fill','rgba(239,93,96,0.18)').attr('opacity',0)
      .transition().delay(dly(500)).duration(dur(1200)).attr('opacity',1);
  }
