...
export async function build(sel, props){
  ...
  pts.forEach(p=>{
    centers.forEach(c=>{ const d=Math.hypot(c.x-p.proj[0], c.y-p.proj[1]); if(d<80) c.values.push(p.burden); });
  });
  ...
  g.append('g').selectAll('text.mlbl').data(pts).join('text').attr('class','mlbl')
    .attr('x',d=>d.proj[0]+8).attr('y',d=>d.proj[1]-8).attr('fill','var(--ink)').attr('font-size',11).attr('font-weight',600)
